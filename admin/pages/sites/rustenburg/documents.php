    ...
    <div id="wrapper">
                                ...
                                <div class="col-sm-3">
                                    <label>Group</label>
                                        <select class="form-control legisChoose" name="docs" id= "docs_id" onchange="return secGroup();">
                                            ...
                                            <option value="pms" <?php if(@$_POST['docs'] == 'annual budget') { ?> selected <?php } ?>>PMS</option>
                                            <option value="bylaws" <?php if(@$_POST['docs'] == 'bylaws_tarrifs') { ?> selected <?php } ?>>By-laws</option>
                                            <option value="annual_reports" <?php if(@$_POST['docs'] == 'performance report') { ?> selected <?php } ?>>Annual reports</option>
                                            <option value="budget" <?php if(@$_POST['docs'] == 'performance agreement') { ?> selected <?php } ?>>Budget</option>


						<option value="policies" <?php if(@$_POST['docs'] == 'policies') { ?> selected <?php } ?>>Policies</option>
                                            <option value="idp" <?php if(@$_POST['docs'] == 'idp') { ?> selected <?php } ?>>IDP</option>
                                            <option value="council_minutes" <?php if(@$_POST['docs'] == 'sdbip') { ?> selected <?php } ?>>Council minutes</option>
						<option value="sdf" <?php if(@$_POST['docs'] == 'sdf') { ?> selected <?php } ?>>SDF</option>
						<option value="reports" <?php if(@$_POST['docs'] == 'other') { ?> selected <?php } ?>>Reports</option>
						<option value="sdbip" <?php if(@$_POST['docs'] == 'led') { ?> selected <?php } ?>>SDBIP</option>
							<option value="roll" <?php if(@$_POST['docs'] == 'roll') { ?> selected <?php } ?>>Valuation roll</option>
								<option value="land_use" <?php if(@$_POST['docs'] == 'land_use') { ?> selected <?php } ?>>Land use management</option>
								<option value="vision" <?php if(@$_POST['docs'] == 'vision') { ?> selected <?php } ?>>Vision 2040</option>
								<option value="street_maps" <?php if(@$_POST['docs'] == 'street_maps') { ?> selected <?php } ?>>Street Maps</option>
								<option value="ward_maps" <?php if(@$_POST['docs'] == 'ward_maps') { ?> selected <?php } ?>>Ward Maps</option>
								<option value="zoning_infrastructure" <?php if(@$_POST['docs'] == 'zoning_infrastructure') { ?> selected <?php } ?>>Zoning and Infrastructure</option>
								<option value="information" <?php if(@$_POST['docs'] == 'information') { ?> selected <?php } ?>>Information and Contacts</option>
								<option value="rlm_libraries" <?php if(@$_POST['docs'] == 'rlm_libraries') { ?> selected <?php } ?>>RLM Libraries</option>
	 ...
	 <?php
                ...
                if(isset($_POST['save']) && $_FILES['myfile']['size']>0){
                    ...
                    $group = htmlspecialchars(strip_tags($_POST['docs']));
